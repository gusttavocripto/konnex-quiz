...
const QUESTIONS = [
  {
    q: "What is Konnex?",
    opts: ["A permissionless on-chain marketplace where robots use AI to perform and verify physical work","A centralized robot rental platform powered by Ethereum","A DeFi lending protocol for robotics hardware financing","A Layer 2 blockchain built specifically for IoT devices"],
    a: 0,
    explanation: "Konnex is the permissionless market for robotic AI and verified physical work — a single on-chain fabric with stablecoin settlement.",
  },
  {
    q: "What is the name of Konnex's unique consensus mechanism used to verify completed physical tasks?",
    opts: ["Proof of Stake (PoS)","Proof of Physical Work (PoPW)","Proof of Authority (PoA)","Proof of History (PoH)"],
    a: 1,
    explanation: "PoPW (Proof of Physical Work) is Konnex's mechanism where validators verify sensor data, video, and GPS logs to confirm real-world task completion.",
  },
  {
    q: "What is the name of the standardized language that allows robots from different manufacturers to communicate tasks on Konnex?",
    opts: ["RobotML","Universal Task Language (UTL)","OpenVLA Schema","TaskScript v2"],
    a: 1,
    explanation: "UTL (Universal Task Language) describes tasks in JSON format, eliminating vendor lock-in and allowing any robot or AI model to read instructions uniformly.",
  },
  {
    q: "Which smart contract in the Konnex protocol manages escrow funds, deadlines, and penalty conditions for each task?",
    opts: ["StakeVault","PayoutRouter","TaskRegistry","RobotIdentity"],
    a: 2,
    explanation: "TaskRegistry records each task's escrow funds, deadlines, and penalty conditions. PayoutRouter handles settlement; StakeVault manages validator stakes.",
  },
  {
    q: "What are the primary utilities of the $KNX token within the Konnex protocol?",
    opts: ["Used only to pay gas fees on Ethereum mainnet","Network fees, validator staking collateral, and governance voting rights","Exclusive medium of exchange for all task payments and settlements","A governance-only token with no fee utility"],
    a: 1,
    explanation: "$KNX is used for network fees, security collateral for validators, and future governance voting. Task settlements are done in stablecoins for price stability.",
  },
  {
    q: "In what currency are all task payments, escrows, penalties, and rewards processed on Konnex?",
    opts: ["KNX token","ETH (Ether)","Stablecoins (e.g., USDC)","BTC (Bitcoin)"],
    a: 2,
    explanation: "Stablecoins are the default settlement currency for all tasks, escrows, penalties, and rewards — ensuring predictable economics for all parties.",
  },
  {
    q: "What type of network communication protocol does Konnex use for low-latency robot-to-robot messaging?",
    opts: ["HTTP/2 REST API","QUIC/libp2p P2P mesh network","WebSockets over TCP","gRPC over TLS"],
    a: 1,
    explanation: "Konnex uses a low-latency P2P mesh network based on QUIC/libp2p for robot-to-robot communication, ensuring fast and reliable task coordination.",
  },
  {
    q: "According to the official Konnex roadmap, which phase focuses on Industrial Pilots with warehouse and agriculture trials?",
    opts: ["Phase 0 — TestNet Sprint","Phase 1 — MainNet MVP","Phase 2 — Industrial Pilot","Phase 3 — Global Motion Ledger"],
    a: 2,
    explanation: "Phase 2 (12–24 months) covers warehouse & agriculture field trials, university rover fleets, BondMatrix v1, and swarm composite bids.",
  },
  {
    q: "How much did Konnex raise in its strategic funding round announced in January 2026?",
    opts: ["$5 million","$50 million","$100 million","$15 million"],
    a: 3,
    explanation: "Konnex secured $15M in January 2026 from strategic partners including Cogitent Ventures, Liquid Capital, Leland Ventures, Covey Network, M77 Ventures, and Block Maven LLC.",
  },
  {
    q: "What is the estimated size of the physical labor economy that Konnex aims to transform?",
    opts: ["~$500 billion","~$1 trillion","~$25 trillion","~$150 billion"],
    a: 2,
    explanation: "Konnex targets the entire real-world labor economy, estimated at approximately $25 trillion — spanning logistics, manufacturing, agriculture, infrastructure, and services.",
  },
];
...
export default function App() {
  ...
  const advance = () => {
    if (qIdx + 1 >= QUESTIONS.length) {
      saveScore(name, scoreRef.current);
      setScreen("result");
    } else {
      setQIdx(i => i + 1);
    }
        ...
        {screen === "result" && (
          <div style={{ width:"100%", textAlign:"center" }}>
            ...
            <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
              <button className="btn btn-primary"   onClick={()=>setScreen("board")}>🏆 View Leaderboard</button>
              <button className="btn btn-secondary" onClick={()=>{ setScore(0); scoreRef.current=0; setResults([]); setQIdx(0); setScreen("quiz"); }}>↺ Try Again</button>
